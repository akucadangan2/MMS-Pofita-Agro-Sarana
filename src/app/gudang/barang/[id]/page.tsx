...
export default async function SatuanBarangGudangPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      ...
      <form action={tambahSatuan} className="mb-6 flex flex-wrap items-end gap-3 rounded-xl border bg-white p-4 shadow-sm">
        <input type="hidden" name="itemId" value={id} />
        <div>
          <label className="mb-1 block text-xs text-slate-500">Nama Satuan</label>
          <input name="namaSatuan" required placeholder="BOX" className="w-32 rounded-lg border px-3 py-2 text-sm" />
        </div>
        <div>
          <label className="mb-1 block text-xs text-slate-500">1 satuan ini = berapa {item.satuan_dasar}?</label>
          <input type="number" name="faktorKonversi" required min={0.01} step="any" className="w-32 rounded-lg border px-3 py-2 text-sm" />
        </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 text-left text-slate-500">
            <tr>
              ...
              <th className="px-4 py-3 font-medium">Konversi ke {item.satuan_dasar}</th>
          ...
          <tbody>
            {satuan.map((s) => (
              <tr key={s.id} className="border-t">
                <td className="px-4 py-3">{s.nama_satuan}</td>
                <td className="px-4 py-3">1 {s.nama_satuan} = {s.faktor_konversi} {item.satuan_dasar}</td>
